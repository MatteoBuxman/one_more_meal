...
<div
  ...
  onclick={()=> (document.getElementById(meal_configuration.uuid) as HTMLDialogElement).showModal()}
...
<dialog id={meal_configuration.uuid} class="modal relative font-lexend">
  ...
  >
    ...
    <div class="flex gap-4 w-full items-center">
      {#if meal_configuration.image}
        <img
          src={meal_configuration.image}
          alt={meal_configuration.name}
          class="size-14 rounded-lg flex-shrink-0"
        />
      {:else}
        <svg
      ...
      <div class="flex flex-col ">
        ...
        <h4 class="font-light text-sm">{meal_configuration.description || "No description."}</h4>
      </div>
    </div>
    <div class="flex flex-col gap-2 text-sm mt-4">
        <h3 class="font-light text-gray-500">Meal ID <span class="font-semibold">{meal_configuration.uuid}</span></h3>
        <h3 class="font-light text-gray-500">Created on {formatDateTime(meal_configuration.created_at)}</h3>
        <StatusBadge status={meal_configuration.status} />
    </div>
    <div class="mt-5">
    ...
    <div class="flex items-center justify-center mt-10">
        {#if meal_configuration.completion_image}
            <img loading="lazy" src={meal_configuration.completion_image} alt="Order completion" class="aspect-[9/16]" />
